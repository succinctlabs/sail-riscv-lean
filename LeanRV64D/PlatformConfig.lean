...
/-- Type quantifiers: k_ex1288114_ : Bool -/
...
def architecture (priv : Privilege) : SailM Architecture := do
  (architecture_bits_backwards
    (← do
      ...
      | .VirtualSupervisor =>
        (internal_error "core/sys_regs.sail" 294 "Hypervisor extension not supported")))
...
/-- Type quantifiers: k_ex1289496_ : Bool, width : Nat, width ∈ {1, 2, 4, 8} -/
...
-- SP1: no CLINT timer device. Upstream default `true`; Sail config `platform.clint.supported`.
...
-- SP1: no test-signature region. Upstream default `true`; Sail config
-- `platform.simple_interrupt_generator.supported`.
